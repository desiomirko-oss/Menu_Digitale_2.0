const VERSION = "12.1-SUPER-MOTORE-COMPLETE";
...
function applyConfig() {
    ...
    if (getVal('App_Bg_Type', 'color').toLowerCase().trim() === 'image' && bgImgUrl !== '') {
    ...
    const defaultF = getVal('Subtitle_Color', '#6b7280');
    root.style.setProperty('--filter-bg', parseColor(getVal('Filter_Bg_Color', 'transparent')));
    root.style.setProperty('--filter-text', parseColor(getVal('Filter_Text_Color', defaultF)));
    root.style.setProperty('--filter-active-bg', parseColor(getVal('Filter_Active_Bg_Color', defaultF)));
    root.style.setProperty('--filter-active-text', parseColor(getVal('Filter_Active_Text_Color', '#ffffff')));

    const mBEn = isTruthy(getVal('Macro_Border_Enable', 'FALSE'));
    root.style.setProperty('--macro-border', mBEn ? `${getVal('Macro_Border_Width', '1px')} solid ${parseColor(getVal('Macro_Border_Color', '#e5e7eb'))}` : 'none');
    const cBEn = isTruthy(getVal('Cat_Border_Enable', 'FALSE'));
    root.style.setProperty('--cat-border', cBEn ? `${getVal('Cat_Border_Width', '1px')} solid ${parseColor(getVal('Cat_Border_Color', '#e5e7eb'))}` : 'none');
    const iBEn = isTruthy(getVal('Item_Border_Enable', 'FALSE'));
    root.style.setProperty('--item-border', iBEn ? `${getVal('Item_Border_Width', '1px')} solid ${parseColor(getVal('Item_Border_Color', '#e5e7eb'))}` : 'none');
...
async function fetchMenu() {
    ...
    try {
        ...
        for(let i=1; i<rows.length; i++){
            ...
            if(c.length >= 3 && c[0]) fullData.push({ 
        ...
        if (levels === 1) { navigationStack = ['page-items']; renderLevel3('', ''); } 
        else if (levels === 2) { navigationStack = ['page-categories']; renderLevel2(''); } 
        else { navigationStack = ['page-macro']; renderLevel1(); }
    } catch(e) { console.error(e); }
}
...
function renderLevel2(m) {
    ...
    if (!navigationStack.includes('page-categories')) navigationStack.push('page-categories');
...
function renderLevel3(m, c, isF = false) {
    ...
    let all = (m === '' && c === '') ? fullData : (m === '' ? fullData.filter(i => i.cat === c) : fullData.filter(i => i.macro === m && i.cat === c));
    
    if (!isF) {
        document.getElementById('sub-header-title').innerText = c || m || 'Menu';
        let fHtml = '';
        const tags = ['noalc', 'gf', 'vegan', 'veg', 'bio'];
        tags.forEach(t => { if(all.some(i => isTruthy(i[t]))) fHtml += `<button onclick="toggleFilter('${t}')" id="btn-${t}" class="filter-btn">${t==='gf'?'Senza Glutine':t==='noalc'?'Analcolico':t.charAt(0).toUpperCase()+t.slice(1)}</button>`; });
        document.getElementById('sub-header-filters').innerHTML = fHtml;
    }

    ['gf', 'vegan', 'veg', 'noalc', 'bio'].forEach(f => { const b = document.getElementById(`btn-${f}`); if(b) activeFilters.includes(f) ? b.classList.add('active') : b.classList.remove('active'); });
    ...
    items.forEach(i => {
        ...
        if(isTruthy(i.bio)) b += `<span class="badge badge-bio">Bio</span>`;
    ...
    if (!isF && !navigationStack.includes('page-items')) navigationStack.push('page-items');
    showPage('page-items');
}

function openItemDetails(id) {
    const item = fullData.find(x => x._id === id); if (!item) return;
    ...
    if(isTruthy(item.bio)) b += `<span class="badge badge-bio">Bio</span>`;
...
function showPage(p) {
    ...
    if (backBtn) p === navigationStack[0] ? backBtn.classList.remove('active') : backBtn.classList.add('active');
