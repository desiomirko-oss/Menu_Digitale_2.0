const VERSION = "11.9-MASTER-FILTERS-BIO";
...
function applyConfig() {
    ...
    if (bgType === 'image' && getVal('App_Bg_Image_URL', '')) root.style.setProperty('--app-bg-image', `url('${getVal('App_Bg_Image_URL', '')}')`);
    ...
    root.style.setProperty('--filter-text', parseColor(getVal('Filter_Text_Color', '#6b7280')));
    root.style.setProperty('--filter-active-bg', parseColor(getVal('Filter_Active_Bg_Color', '#6b7280')));
...
async function fetchMenu() {
    ...
    try {
        ...
        for(let i=1; i<rows.length; i++){
            ...
            // 🆕 Mappatura colonne aggiornata: Bio in colonna K (c[10])
            if(c.length >= 3 && c[0]) fullData.push({ 
                _id: i, macro: c[0], cat: c[1], name: c[2], desc: c[3], price: c[5], 
                gf: c[6], vegan: c[7], veg: c[8], noalc: c[9], bio: c[10],
                active: c[11]||'TRUE', photo: c[12], ar: c[13], details: c[14] || '' 
            });
        }
...
function renderLevel3(m, c, isF = false) {
    ...
    if (!isF) {
        ...
        let fHtml = '';
        const list = m.toLowerCase().match(/bevand|bebid|drink/) ? ['noalc', 'gf', 'bio'] : ['gf', 'vegan', 'veg', 'bio'];
        list.forEach(f => { if(all.some(i => isTruthy(i[f]))) fHtml += `<button onclick="toggleFilter('${f}')" id="btn-${f}" class="filter-btn">${f==='gf'?'Senza Glutine':f==='noalc'?'Analcolico':f==='bio'?'Bio':f.charAt(0).toUpperCase()+f.slice(1)}</button>`; });
    ...
    items.forEach(i => {
        ...
        container.innerHTML += `<div class="menu-card ${hasD?'clickable-card':''}" ${hasD?`onclick="openItemDetails(${i._id})"`:''}><div class="item-card"><div style="flex-grow:1;"><div class="item-name notranslate">${escapeHTML(i.name)}</div><div class="item-desc">${escapeHTML(i.desc)}</div><div class="item-price notranslate">${escapeHTML(i.price)}</div></div>${i.photo ? `<img src="${escapeHTML(i.photo)}" class="item-photo" style="margin-left:10px;" loading="lazy">` : ''}</div>${badgeRow}</div>`;
    });
...
function showPage(p) {
    ...
    pageIds.forEach(id => { const el = document.getElementById(id); if(el) el.classList.add('hidden'); });
